...
> 唯一合同）与 AIRADAR 当时架构（爬取 + AI 管线在本地 Mac，整库同步腾讯云只读展示；
> 腾讯云已于 2026-08-13 停机换成美国 greenvps，本文涉及机房位置的推论见正文标注）。
>
...
- 部署：**爬取与 AI 管线在本地 Mac**，`sync_db_to_server.sh` 整库替换到线上；
  云端只读展示
  > `[理由已失效 2026-08-13]` 原文这里写的是"整库替换到腾讯云……大陆服务器访问不了
  > GitHub/X，**这个架构不能变**"。腾讯云已停机，线上换成美国的 greenvps，
  > **它能直连 GitHub 和 X**（当晚实测 github.com 200、pbs.twimg.com TLS 可达）。
  > 也就是说"不能变"的那个理由没有了——pipeline 现在**技术上可以**迁到服务器，
  > 摆脱"Mac 必须开机才有新数据"的约束。
  >
  > 但这只是说约束解除了，不等于该迁：迁过去要重新安排 API 密钥、调度器、成本归属，
  > 而且本地跑还有"数据主副本在自己机器上"的好处。列为待评估，不是待办。
...
**改 SP topics 时记得同步**（与 handle 清单同一条约定，handle 清单现为 22 个）。

关键调优（同日）：话题搜索从 Latest 切到 **X 的 Top 热门排序**——Latest 模式
抓回的是「过了赞数门槛的最新流水账」，Top 才是事件下真正的热帖（实测 U卡
话题一轮就是 OKX 开卡 645 赞、众安银行 600 赞这类）。当前话题：`AI热点`、
`U卡推荐`、`eSIM推荐`。话题名称是 SP/AR 两端共同使用的筛选标识；改名时必须
同步更新 AR 默认清单并迁移 `x_tweets.topics` 与 `payload.topics` 历史数据。
...
- ~~**云端图片**~~ `[限制可能已解除 2026-08-13]`：原文如下——

  > 推文配图与头像都指向 pbs.twimg.com，走 `/api/image-proxy` 代理，
  > 而生产的 web 服务器在腾讯云、出不去 X 的 CDN——**云端部署后推文图片会加载
  > 失败**（ArticleImage 已有优雅降级：失败显示占位并引导「查看原推」）。真要解决
  > 得在本地同步时把图片抓下来随库/静态资源上云，留待有需求时做。

  这个结论的前提是"服务器在腾讯云"。腾讯云已于 2026-08-13 停机，线上换成美国的
  greenvps，**它能直连 pbs.twimg.com**（当晚实测：TLS 握手 133ms 内完成，返回的 404
  来自 twimg 自身而非网络层，因为测试用的图片路径是编的）。所以这条限制大概率已经
  自动消失，"把图片抓下来上云"那个方案也就不必做了。
  ⚠️ 但**没有端到端验证过真实推文图片在页面上的加载**——只验证了网络可达性。
  真要确认，去 `/x` 页面看图是否正常显示。
