...
def scoring_system_prompt() -> str:
    ...
    return (
        ...
        "公司归属必须有原文中的明确表述支撑：不得仅凭产品名、作者姓名、账号"
        "昵称或常识推断作者代表某家公司，不得把第三方作者写成‘某公司官方’；"
        "原文没有明确公司归属时，使用原文给出的作者名、账号名或‘发布者’作主语，"
        "不要自行补充公司名。title_zh、one_line_summary、summary_zh、reason_zh 和"
        "action_zh 都必须遵守这一归属边界；"
        "禁止编造原文没有的结论、数字或因果关系，"
