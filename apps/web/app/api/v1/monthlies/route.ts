...
export const GET = handleV1(async (request) => {
  ...
  const entries = await loadPeriodArchive("monthly");
  ...
  return ok({
    ...
    items: page.map((entry) => ({
      ...
      links: {
        radar: new URL(`/${PERIOD_META.monthly.page}/${entry.period_key}`, siteUrl).toString(),
        api: new URL(`/api/v1/${PERIOD_META.monthly.collection}/${entry.period_key}`, siteUrl).toString(),
      },
    })),
  }, CACHE.periodIndex);
});
